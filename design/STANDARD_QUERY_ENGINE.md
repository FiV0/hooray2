...
| `RelationPattern` | Represents a materialized relation, including external and nested-plan inputs. Internally, a relation is represented by a trie, meaning that, for any prefix in variable order, only the next variable can be introduced. Both proposal and validation read bound variables in the variable order of the relation wrapped by the pattern; the incoming `BindingSet` may arrange those columns in any order and may interleave unrelated columns. For proposal, the bound variables followed by `added` must form a prefix of the wrapped relation's variables. Validation requires the bound variables themselves to form such a prefix. |
