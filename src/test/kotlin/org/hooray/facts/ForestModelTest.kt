 ...
 * facts as a set of tuples. Data comes in two width regimes — uniform four-byte terms,
 * which drive the width-specialized kernel paths, and mixed-width terms, which drive
...
class ForestModelTest {
    ...
    private fun regimes(count: Int, arity: Int, seed: Long): List<Pair<String, List<TestFact>>> =
        listOf(
            Pair("fixed", fixedTuples(count, arity, seed)),
    ...
    fun `build and merge match the model`() {
        for ((name, xs) in regimes(500, 3, 1)) {
            val ys = if (name == "fixed") fixedTuples(500, 3, 2) else mixedTuples(500, 3, 2)
            val fx = forestOf(xs)
            val fy = forestOf(ys)
    ...
    fun `semijoin and antijoin match the model`() {
        for ((name, xs) in regimes(500, 2, 7)) {
            val keys = if (name == "fixed") fixedTuples(50, 1, 8) else mixedTuples(50, 1, 8)
            val fx = forestOf(xs)
    ...
    fun `semijoin against several forests uses their union`() {
        for ((name, xs) in regimes(300, 2, 9)) {
            val (ys, zs) = if (name == "fixed") {
                Pair(fixedTuples(30, 1, 10), fixedTuples(30, 1, 11))
            } else {
    ...
    fun `filter matches the model`() {
        for ((name, xs) in regimes(500, 3, 3)) {
            ...
            val bothActual = fx.filter(listOf(Pair(1, intsToBytes(lit))), listOf(listOf(0, 2)))
    ...
    fun `fact set advance accumulates distinct facts`() {
        val regimes = listOf(
            Pair("fixed", List(4) { fixedTuples(200, 2, 20L + it) }),
    ...
    fun `zero arity forests merge to the single empty fact`() {
        ...
        assertEquals(setOf(emptyList<List<Int>>()), extract(empty))
