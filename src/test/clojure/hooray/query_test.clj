...
(deftest test-basic-query
  ...
  (t/testing "Negate query based on subsequent non-matching clause"
    (t/is (= #{} (h/q '{:find [e]
                        :where [[e :name "Ivan"]
                                [e :last-name "Ivanov-does-not-match"]]} (h/db *node*)))))

  (t/testing "Can query for multiple results"
    (t/is (= #{["Ivan"] ["Petr"]}
             (h/q '{:find [name] :where [[e :name name]]} (h/db *node*)))))

  (h/transact *node* [{:db/id :smith :name "Smith" :last-name "Smith"}])

  (t/testing "Can query across fields for same value"
    (t/is (= #{[:smith]}
             (h/q '{:find [p1] :where [[p1 :name name]
                                       [p1 :last-name name]]} (h/db *node*)))))
...
#_ (t/deftest test-circular-dependencies
  (t/is (empty?
         (xt/q (xt/db *api*)
               '{:find [bah]
                 :in [$ bah]
...
#_ (t/deftest test-join-and-seek-bugs
  (fix/transact! *api* (fix/people [{:xt/id :ivan :name "Ivan" :last-name "Ivanov"}
                                    {:xt/id :petr :name "Petr" :last-name "Petrov" :mentor :ivan}]))
...
#_ (t/deftest test-queries-with-variables-only
...
#_ (t/deftest test-index-unification
  (fix/transact! *api* (fix/people [{:xt/id :ivan :name "Ivan" :last-name "Ivanov"}
                                    {:xt/id :petr :name "Petr" :last-name "Petrov" :mentor :ivan}]))
...
#_ (t/deftest test-get-valid-time
  (let [now #inst "2000"
        later1 #inst "2050"
        later2 #inst "2100"
        later3 #inst "2150"
...
#_ (t/deftest test-byte-array-values
...
#_ (t/deftest test-collection-returns
  (t/testing "vectors"
    (t/is (= #{[1]
               [2]}
             (xt/q (xt/db *api*)
...
#_ (t/deftest test-tuple-returns
  (t/is (= #{[1 2]}
...
#_ (t/deftest test-relation-returns
  (t/is (= #{[1 2]
             [3 4]}
           (xt/q (xt/db *api*) '{:find [x y]
...
#_ (t/deftest test-mutiple-values
...
#_ (t/deftest test-range-join-order-bug-348
...
#_ (t/deftest test-or-range-vars-bug-949
...
#_ (t/deftest test-query-limits-bug-71
  (dotimes [i 10]
    (fix/transact! *api* (fix/people [{:name "Ivan" :last-name "Ivanov"}
                                      {:name "Petr" :last-name "Petrov"}
...
#_ (t/deftest test-self-join-bug-93
...
#_ (t/deftest test-arguments-bug-247
  (t/is (= #{} (xt/q (xt/db *api*)
                     '{:find [?x]
                       :where [[?x :name]]
                       :args [{:?x "Clojure"}]}))))

#_ (t/deftest test-npe-arguments-bug-314
  (t/is (= #{} (xt/q (xt/db *api*)
                     '{:find [e]
                       :where
                       [[e :xt/id _]]
...
#_ (t/deftest test-optimise-range-constraints-bug-505
...
#_ (t/deftest test-non-entity-id-lookup-issue-287
...
#_ (t/deftest test-query-and-match
...
#_ (t/deftest test-bitemp-query-from-indexing-temporal-data-using-existing-b+-trees-paper
...
#_ (defn- populate-datascript-test-db []
...
#_ (t/deftest test-rules-with-recursion-1
...
#_ (t/deftest test-rules-with-recursion-2
  (fix/transact! *api* [{:xt/id 1 :follow 2}
                        {:xt/id 2 :follow 3}])
  (t/is (= (xt/q (xt/db *api*)
                 '{:find [?e1 ?e2]
...
#_ (t/deftest test-calling-rule-twice-44
...
#_ (t/deftest test-mutually-recursive-rules
...
#_ (defn- even-or-nil? [x]
  (when (even? x)
    x))

#_ (t/deftest data-script-test-query-fns
...
#_ (t/deftest datascript-test-predicates
  (fix/transact! *api* [{:xt/id 1 :name "Ivan" :age 10}
                        {:xt/id 2 :name "Ivan" :age 20}
                        {:xt/id 3 :name "Oleg" :age 10}
                        {:xt/id 4 :name "Oleg" :age 20}])
...
#_ (t/deftest datascript-test-issue-180
...
#_ (defn sample-query-fn [] 42)

#_ (t/deftest datascript-test-symbol-resolution
  (let [db (xt/db *api*)]
    (t/is (= #{[42]} (xt/q db
                           '{:find [?x]
...
#_ (t/deftest test-projection-exprs
...
#_ (t/deftest test-aggregate-exprs
...
#_ (t/deftest test-can-bind-function-returns-to-falsy
...
#_ (t/deftest test-can-use-any-value-as-entity-id
...
#_ (t/deftest test-racket-datalog-tutorial
  ;; parent(john,douglas).
...
#_ (t/deftest test-racket-datalog-path
...
#_ (t/deftest test-racket-datalog-revpath
  ;; edge(a, b). edge(b, c). edge(c, d). edge(d, a).
  (fix/transact! *api* [{:xt/id :a :edge :b}
                        {:xt/id :b :edge :c}
                        {:xt/id :c :edge :d}
...
#_ (t/deftest test-racket-datalog-bidipath
  ;; edge(a, b). edge(b, c). edge(c, d). edge(d, a).
  (fix/transact! *api* [{:xt/id :a :edge :b}
                        {:xt/id :b :edge :c}
                        {:xt/id :c :edge :d}
...
#_ (t/deftest test-racket-datalog-sym
  ;; sym(a).
  ;; sym(b).
  ;; sym(c).
...
#_ (t/deftest test-failing-predicates-at-top-level-bug
...
#_ (t/deftest test-literal-rule-arguments-bug-507
...
#_ (t/deftest test-racket-datalog-fib
  ;; fib(0, 0).
  ;; fib(1, 1).
...
#_ (t/deftest test-datalog-paper-sgc
...
#_ (t/deftest test-datalog-paper-stratified-datalog
  ;; d(a, b), d(b, c) d(e, e)
...
#_ (t/deftest test-query-against-empty-database-376
  (let [db (xt/db *api*)
...
#_ (t/deftest test-can-use-cons-in-query-377
...
#_ (t/deftest test-query-keyword-to-entity-tx-351
...
#_ (t/deftest test-order-by-when-not-specified-in-return-418
...
#_ (t/deftest test-query-with-timeout-419
...
#_ (defn long-running-pred [] (Thread/sleep 100) :ivan)

#_ (t/deftest test-query-with-predicate-with-timeout-1654
  (t/is (thrown? TimeoutException
                 (xt/q (xt/db *api*) '{:find [e]
                                       :where [[(xtdb.query-test/long-running-pred) e]]
                                       :timeout 10}))))

#_ (t/deftest test-nil-query-attribute-453
...
#_ (t/deftest test-greater-than-range-predicate-545
  (t/is (empty?
         (xt/q (xt/db *api*)
               '{:find [offset]
...
#_ (t/deftest test-query-result-cardinality-972
...
#_ (t/deftest test-unsorted-args-697
...
#_ (t/deftest prioritises-single-value-in-vars-like-literals-1556
...
#_ (t/deftest can-override-join-order
  (fix/submit+await-tx (->> (for [x (range 1000)]
                              [[::xt/put {:xt/id (str "thing-" x), :type :thing, :v x}]
                               [::xt/put {:xt/id (str "other-thing-" x), :type :other-thing, :v x}]])
                            (apply concat)))
...
#_ (t/deftest test-binds-against-false-arg-885
...
#_ (t/deftest test-unused-args-still-binding-882
...
#_ (t/deftest test-leaf-vars-and-ors
...
#_ (t/deftest test-bound-rule-vars-946
...
#_ (t/deftest test-cardinality-join-order-avoids-cross-product
  (fix/transact! *api* (fix/people
                        (apply concat
                               (for [n (range 100)]
...
#_ (t/deftest resurrecting-doc-1127
  (let [query {:find '[n]
               :where '[[n :name "hello"]
                        [n :age 17]]}]
...
#_ (t/deftest hashing-quoted-lists-1197
...
#_ (t/deftest falsey-values-bind-with-rules
...
#_ (t/deftest closing-node-interrupts-open-snapshots
...
#_ (t/deftest nil-in-entity-position-shouldnt-yield-results-1486
...
#_ (t/deftest literal-nil-value-in-triple-clause-should-only-match-nil-1487
  (fix/submit+await-tx [[::xt/put {:xt/id 1 :foo nil}]
...
#_ (t/deftest picks-more-selective-join-order
...
#_ (defn- date->inverted-long [^Date d]
  (* -1 (.getTime d)))

#_ (defn- inverted-long->date [l]
  (Date. ^Long (* -1 l)))

#_ (t/deftest range-constraint-ordering-behaviours
  (fix/transact!
   *api*
...
#_ (t/deftest circular-deps-test-1523
...
#_ (t/deftest test-rules-binding-1569
...
#_ (defn- test-join-orders [queries
...
#_ (t/deftest test-tpch-join-orders
...
#_ (t/deftest test-watdiv-join-orders
...
#_ (t/deftest test-same-literal-treated-as-separate-variables-1695
...
#_ (t/deftest test-pred-variable-resolution-error-1731
...
#_ (t/deftest handles-start-tx-time-between-txs-1732
...
#_ (t/deftest external-sort-test
...
#_ (t/deftest test-bound-vars-cce-2534
  (t/is (= #{[1]}
           (xt/q (xt/db *api*)
                 '{:find [a]
                   :in [a b]
