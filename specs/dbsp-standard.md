...
2. **Input model** — Each triple pattern is its **own input** (own `Source`),
   even when the same attribute appears in several patterns (self-joins are
   independent inputs). A pattern's matching facts are supplied as **flat
   permuted `ZSet`s**: unlike the current `ZSetIndices` (nested `IndexedZSet`s),
   each "order" is a flat `ZSet` whose tuples are the *same data in a different
   column permutation*. v1 maintains two orders — entity-first (`AEV`) and
   value-first (`AVE`). The planner picks each pattern's order to match its join
   key, so base patterns need **no `map_index` / re-index operator**.
...
|---|---|---|
| `Source` | source | Emits a pattern's per-step delta `ZSet`, fed by an `InputHandle` (chosen permutation) |
...
registered standard query and per pattern**, the pattern's delta facts as flat
`ZSet`s of indexed arrays in two permutations:

- `AEV` order — entity-leading, e.g. tuples `[e v]`.
- `AVE` order — value-leading, e.g. tuples `[v e]`.

(The attribute is constant per pattern and dropped from the tuple.) Each pattern
has one `Source` with one `InputHandle`; the dispatcher `push`es the permutation
the planner assigned that pattern, so its join key is already the leading
column(s). Two orders suffice in v1: a pattern has at most two variables, so its
join key is `e`, `v`, or `{e,v}`. Constant `e`/`v` in a pattern are handled by a
`Filter`; projection to variable columns by a `Map`.
...
- `dbsp/compute-delta!` builds the per-pattern `AEV`/`AVE` delta `ZSet`s,
  `push`es each pattern's planner-chosen permutation through its `InputHandle`,
...
src/main/kotlin/org/hooray/dbsp/
  ...
  operators/
    SourceOp.kt           → per-pattern input handle
...
1. **Permutation set** — `AEV` + `AVE` suffice for v1; the attribute is fixed
   per pattern and a pattern has ≤2 variables.
...
3. **Delta delivery** — per-query `AEV`/`AVE` deltas are computed in
