...
`ExecPattern.count` receives the current per-row proposal table and returns an
updated proposal table. A proposal stores the winning participant index and the
estimated number of rows that participant can produce for the current input row.
Patterns that cannot propose the stage variables return the proposal table
unchanged.
...
2. Walk the current query variable order and normally introduce one variable per
   stage.
3. Allow a stage to introduce multiple variables when a normal one-variable step
   would split an OR branch continuation. Example: an OR-only branch group that
   contains `?x` and `?c` introduces both together.
4. `PlanPattern.groundable(bound)` reports what a pattern could ground from the
   current bound set. The v1 implementation keeps this as Clojure descriptor
   logic rather than a Kotlin interface.
...
7. Treat `or` as a first-class plan pattern. `groundable(bound)` is simply the
   OR variables not yet bound. The stage planner decides when those variables
...
3. If the stage introduces variables, initialize one proposal per input row with
   `idx = -1` and `count = Int.MAX_VALUE`.
4. Fold every participant through `count(input, introduces, proposals)`.
5. Drop rows whose final proposal has no participant or a non-positive count.
6. Group the remaining rows by winning participant `idx`.
7. For each shard, call the winning participant's `propose`, then validate the
   proposed rows with every other participant.
8. Distinct full rows and reorder to `targetVariables`.
...
folds a proposal table over the input rows:

```text
row-index | input row | initial proposal | after P0 | after P1 | final
----------+-----------+------------------+----------+----------+-------
0         | [...]     | -1 / INF         | P0 / 12  | P1 / 3   | P1 / 3
1         | [...]     | -1 / INF         | P0 / 1   | P0 / 1   | P0 / 1
```

Rows with no winning proposal are dropped before proposal. The remaining rows
are partitioned into proposer groups:
...
shard with every other participant and merges the validated shards back into one
`BindingSet`. This is the local `BindingSet` version of Datatoad's "append
counts, partition by best atom, propose, semijoin with others" loop.
...
`or` participates in the same stage shape as every other pattern, but its count
behavior is conservative. In ordinary mixed stages it leaves the proposal table
unchanged. When the stage target covers all OR variables and branch proposal
plans can be built, it may claim rows that no earlier participant claimed by
writing `Proposal(idx, Int.MAX_VALUE)`.
...
1. Determine the seed variables: the variables already present in the input
   `BindingSet`.
2. Determine the OR output variables: all free variables of the `or` pattern
...
5. Inside a branch, the normal staged WCO executor can still introduce variables
   level by level and prune early.
6. A branch may validate seeded variables, introduce missing OR variables, or do
   both.
7. Branch predicates and functions validate only that branch's rows.
8. Each branch returns rows covering the requested target layout, normally
   `seed variables + OR output variables`.
9. The `or` executor unions distinct full rows across branches.
10. Any other OR clauses that overlap the produced variables validate the unioned
...
plan before OR proposal returns.
...
If the next variable in the outer variable order is only groundable through an
OR, the planner introduces all currently unbound variables of that OR together.
For example:
...
by the first branch. Instead, the stage introduces all unbound variables of that
OR and the OR runs each branch to completion for `[?x ?c]`:
...
can, the proposal table lets one OR claim otherwise unclaimed rows with
`Int.MAX_VALUE`, and the other OR clauses validate the proposal output. This
supports OR-only intersections without making OR a cheap mixed-stage proposer.
...
- Planner tests:
  ...
  - proposing `or` with multiple unbound output variables introduces those
    variables in one stage, not split across several outer stages;
  - multiple OR-only clauses use the proposal table to choose one OR and
    validate the resulting rows with the remaining ORs;
...
- A pattern can introduce one or more variables in a single stage, but a
  proposing `or` is never split across several outer stages.
- Multiple OR-only clauses can still be planned by allowing one OR to claim the
  stage proposal and the others to validate as semijoins.
