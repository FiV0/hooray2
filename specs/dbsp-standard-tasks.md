...
ends by running its Verify step; commit only when explicitly asked. Paths are relative to repo root;
...
- [ ] **T4 — Input source / `Filter` / `Map` operators**
  - Acceptance: `Circuit.addInput` provides the InputHandle-backed source stream;
    `FilterOp` (keep entries by key predicate); `MapOp` (key-transform,
  ...
  - Files: `K/Circuit.kt`, `K/FilterOp.kt`, `K/MapOp.kt`, `KT/LinearOpsTest.kt`.
...
- [ ] **T13 — Shared `AEV`/`AVE` index delta builder**
  - Acceptance: from `tx-data` + db-before, build shared `AEV`/`AVE` index
    delta `ZSet<Tuple>`s shaped as `[a e v]` and `[a v e]`; each pattern filters
    its own attribute and constants before projecting variables. Reuse
    `hooray.incremental` retract/cardinality logic.
