...
| **B1** | Linear/IO operators — input source handles, `FilterOp`, `MapOp` | Kotlin | A |
...
| **F** | Delta path — `compute-delta!`: build shared `AEV`/`AVE` index deltas, push each pattern's planned order via `InputHandle`, `step`, read `OutputHandle`, format result | Clojure `hooray.dbsp` | E |
...
**M2 — Operators (B1, B2).**
Input sources/`FilterOp`/`MapOp`, then the primitives `IntegrateOp`,
...
Compile a plan into a Kotlin `Circuit` via interop; build shared `AEV`/`AVE`
index delta `ZSet`s from `tx-data` (adapt the retract/cardinality logic of
`hooray.incremental/calc-zset-indices`, but emit flat `[a e v]` / `[a v e]`
`ZSet`s); wire `compute-delta!`.
→ Checkpoint **CP5**.
...
5. **Delta construction divergence** — flat `AEV`/`AVE` index deltas must agree
   with what the WCOJ path derives (retracts, cardinality, prev-value).
